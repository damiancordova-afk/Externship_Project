"""
build_sources.py — KEYLESS discovery of career-fair source URLs.

For each target school it:
  ...
  3. fetches + validates them (must return real content mentioning a career fair)
  4. shallow-crawls a valid landing page for deeper fair/event links
  5. writes the good URLs into fair_sources.json (render:true) for the scraper

No API keys. Run:  python3 build_sources.py
Deps: requests, beautifulsoup4  (pip install -r scripts/requirements.txt)
"""
...
def directory_schools() -> list[str]:
    """Every school currently shown in the directory (so the sweep covers all
    ~485, not just the curated targets). Skips unresolvable ambiguous buckets."""
    try:
        html = open("campus-recruiting-directory.html", encoding="utf-8").read()
